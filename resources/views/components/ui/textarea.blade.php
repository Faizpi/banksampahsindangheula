...
@php
    ...
    $hintId = $hint ? "{$textareaId}-hint" : null;
    $errorId = $error ? "{$textareaId}-error" : null;
    $describedBy = collect([$hintId, $errorId])->filter()->implode(' ');
@endphp

<div class="space-y-2">
    <label for="{{ $textareaId }}" class="block text-label text-text-primary">{{ $label }}</label>
    <textarea
        id="{{ $textareaId }}"
        name="{{ $name }}"
        ...
        {{ $attributes->except(['id'])->class([
            'min-h-30 w-full resize-y rounded-md border bg-surface px-4 py-3 text-body text-text-primary shadow-xs transition duration-180 placeholder:text-text-secondary hover:border-forest-600 focus:border-focus disabled:cursor-not-allowed disabled:border-border disabled:bg-disabled-bg disabled:text-text-secondary',
