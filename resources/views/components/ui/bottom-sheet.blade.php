...
<div
    ...
    x-cloak
    x-show="open"
    x-on:open-bottom-sheet.window="if ($event.detail?.id === @js($componentId)) openModal($event.detail?.invoker)"
    x-on:keydown.escape.window="closeModal()"
    {{ $attributes->class('fixed inset-0 z-dialog') }}
>
    ...
    <section
        ...
        class="absolute inset-x-0 bottom-0 z-dialog max-h-[85dvh] overflow-y-auto rounded-t-lg border border-border bg-surface p-5 shadow-sm transition duration-240 ease-standard motion-reduce:transform-none md:inset-x-4 md:bottom-4 md:mx-auto md:max-w-2xl md:rounded-lg"
    >
        ...
        <div class="pt-4 text-body">{{ $slot }}</div>
