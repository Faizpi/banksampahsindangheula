    ...
    <div class="space-y-6">
        ...
        @if ($canRunBackup)
            <section id="backup" class="scroll-mt-32 rounded-xl border border-gray-200 bg-white p-5 shadow-sm" aria-labelledby="backup-title"><h2 id="backup-title" class="text-lg font-semibold text-gray-950">Cadangan</h2><p class="mt-1 text-sm text-gray-600">Catat pasangan lokasi, checksum, ukuran, status, dan retensi. Berkas cadangan dibuat melalui SOP/deployment.</p><form wire:submit="recordBackupMetadata" class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <label class="block text-sm font-medium text-gray-800">Ukuran database (byte)<input wire:model="backupDatabaseSizeBytes" placeholder="Contoh: 12400000" required inputmode="numeric" class="mt-1 min-h-11 w-full rounded-lg border-gray-300 text-sm shadow-sm"></label>
        ...
        @if ($canViewBackups)
            <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm" aria-labelledby="backups-title"><h2 id="backups-title" class="text-lg font-semibold text-gray-950">Metadata cadangan terakhir</h2>@if ($backups->isEmpty())<p class="mt-3 text-sm text-gray-600">Belum ada metadata cadangan yang dapat ditampilkan.</p>@else
                <div class="mt-4 space-y-3 md:hidden">@foreach ($backups as $backup)<details class="rounded-lg border border-gray-200 p-4"><summary class="cursor-pointer list-none"><span class="font-medium">Cadangan #{{ $backup->id }}</span><span class="ml-2 text-sm text-gray-600">{{ \App\Support\StatusLabel::for($backup->status) }}</span><span class="mt-1 block text-xs text-gray-500">{{ $backup->created_at?->setTimezone('Asia/Jakarta')->format('d M Y H:i') ?? 'Tanggal tidak tersedia' }}</span></summary><dl class="mt-3 space-y-2 text-sm"><div class="flex justify-between gap-4"><dt class="text-gray-600">Database</dt><dd class="text-right">{{ $backup->database_location_alias }}<br>{{ $backup->humanSize($backup->database_size_bytes) }}</dd></div><div class="flex justify-between gap-4"><dt class="text-gray-600">Media</dt><dd class="text-right">{{ $backup->media_location_alias }}<br>{{ $backup->humanSize($backup->media_size_bytes) }}</dd></div><div class="flex justify-between gap-4"><dt class="text-gray-600">Verifikasi</dt><dd>{{ $backup->restore_verification_result?->value ? \App\Support\StatusLabel::for($backup->restore_verification_result) : 'Belum diuji' }}</dd></div><div class="flex justify-between gap-4"><dt class="text-gray-600">Retensi</dt><dd>{{ $backup->retention_until->setTimezone('Asia/Jakarta')->format('d M Y H:i') }}</dd></div><div class="border-t border-gray-100 pt-2 text-xs text-gray-500"><dt>Checksum database</dt><dd class="break-all">{{ $backup->database_sha256 }}</dd><dt class="mt-1">Checksum media</dt><dd class="break-all">{{ $backup->media_sha256 }}</dd></div></dl></details>@endforeach</div>
                <div class="mt-4 hidden overflow-x-auto md:block"><table class="min-w-full text-left text-sm"><caption class="sr-only">Daftar metadata cadangan terbaru</caption><thead class="border-b border-gray-200 text-xs uppercase tracking-wide text-gray-500"><tr><th class="px-3 py-2">ID</th><th class="px-3 py-2">Status</th><th class="px-3 py-2">Database</th><th class="px-3 py-2">Media</th><th class="px-3 py-2">Pemulihan</th><th class="px-3 py-2">Retensi</th></tr></thead><tbody class="divide-y divide-gray-100">@foreach ($backups as $backup)<tr><td class="px-3 py-3 font-medium">{{ $backup->id }}<br><span class="text-xs text-gray-500">{{ $backup->created_at?->setTimezone('Asia/Jakarta')->format('d M Y H:i') }}</span></td><td class="px-3 py-3">{{ \App\Support\StatusLabel::for($backup->status) }}</td><td class="px-3 py-3">{{ $backup->database_location_alias }}<br><span class="text-xs text-gray-500">{{ $backup->humanSize($backup->database_size_bytes) }}</span></td><td class="px-3 py-3">{{ $backup->media_location_alias }}<br><span class="text-xs text-gray-500">{{ $backup->humanSize($backup->media_size_bytes) }}</span></td><td class="px-3 py-3">{{ $backup->restore_verification_result?->value ? \App\Support\StatusLabel::for($backup->restore_verification_result) : 'Belum diuji' }}</td><td class="px-3 py-3">{{ $backup->retention_until->setTimezone('Asia/Jakarta')->format('d M Y H:i') }}</td></tr>@endforeach</tbody></table></div>
            @endif</section>
        @endif
    </div>
</x-filament-panels::page>
