...
DIAGRAMS: list[Diagram] = [
    Diagram("PERENCANAAN & PENGEMBANGAN", "Tahapan Pengembangan Aplikasi", "Menunjukkan siklus pengembangan dari kebutuhan hingga evaluasi penggunaan.", "Tim pengembang, pengelola, petugas, perwakilan warga", [[n("a","Mulai","start")],[n("b","Analisis kebutuhan")],[n("c","Perancangan sistem dan UI")],[n("d","Pengembangan modul")],[n("e","Pengujian")],[n("f","Sesuai kebutuhan?","decision")],[n("g","Penerapan")],[n("h","Evaluasi penggunaan")],[n("i","Selesai","end")]], chain(list("abcdef"))+[Edge("f","g","Ya"),Edge("f","b","Tidak"),Edge("g","h"),Edge("h","i")], "Setiap tahap menghasilkan keluaran yang diperiksa sebelum berlanjut."),
    Diagram("PERENCANAAN & PENGEMBANGAN", "Peta Jalan Pengembangan", "Menunjukkan urutan implementasi ruang lingkup aplikasi yang disetujui.", "Pemerintah desa, pengelola, tim pengembang", [[n("a","Ruang lingkup disetujui","start")],[n("b","Fondasi, akun, wilayah, data sampah")],[n("c","Transaksi, saldo, penjemputan, pencairan, sembako")],[n("d","Target, layanan keliling, estimasi, QR, statistik")],[n("e","Integrasi seluruh modul")],[n("f","Pengujian dan UAT")],[n("g","Sesuai ruang lingkup?","decision")],[n("h","Perbaikan")],[n("i","Peluncuran dan evaluasi","end")]], chain(["a","b","c","d","e","f","g"])+[Edge("g","h","Belum"),Edge("h","e"),Edge("g","i","Ya")], "Seluruh kemampuan dalam ruang lingkup harus lolos pengujian sebelum penerimaan sistem."),
    ...
    Diagram("SISTEM & PENGGUNA", "Diagram Konteks Sistem", "Memperlihatkan pihak yang berinteraksi dengan Bank Sampah Digital.", "Warga, petugas, admin, bendahara, superadmin", [[n("a","Warga\nregistrasi, setoran, saldo, pengajuan","data"),n("b","Petugas\npenimbangan, penjemputan, penyerahan","data")],[n("s","BANK SAMPAH DIGITAL\nSINDANGHEULA","process")],[n("c","Admin\npengguna, harga, persetujuan, laporan","data"),n("d","Bendahara\npembayaran pencairan","data"),n("e","Superadmin\nadmin, role, permission, status akun","data")]], [Edge("a","s"),Edge("b","s"),Edge("s","c"),Edge("s","d"),Edge("s","e")], "Semua akses dibatasi berdasarkan role dan setiap aktivitas penting dicatat."),
    Diagram("SISTEM & PENGGUNA", "Arsitektur Sistem Tingkat Tinggi", "Menunjukkan teknologi utama, web hosting terkelola, komponen sistem, dan aliran data.", "Pengguna aplikasi dan pengelola", [[n("a","Smartphone / tablet / komputer","start")],[n("b","Browser atau PWA")],[n("c","Blade + Livewire + Alpine.js + Tailwind CSS")],[n("d","Laravel 13 + PHP 8.3 atau lebih baru")],[n("e","Otentikasi, transaksi, saldo, laporan")],[n("f","MariaDB kompatibel MySQL","data"),n("g","Storage privat / object storage S3","data")],[n("h","Web hosting terkelola\nhPanel + SSH/SFTP + Composer 2 + cron")],[n("i","Aplikasi Bank Sampah Digital","end")]], chain(["a","b","c","d","e"])+[Edge("e","f"),Edge("e","g"),Edge("f","h"),Edge("g","h"),Edge("h","i")], "Aplikasi memakai arsitektur modular monolith dan menjalankan tugas terjadwal yang diperlukan melalui cron."),
    ...
    Diagram("ADMINISTRASI", "Health Sistem Read-Only", "Menampilkan hasil pemeriksaan komponen aplikasi tanpa tindakan perubahan.", "Admin berwenang", [[n("a","Admin membuka halaman Health","start")],[n("b","Periksa koneksi basis data")],[n("c","Periksa penyimpanan privat")],[n("d","Periksa status scheduler")],[n("e","Periksa status queue")],[n("f","Tampilkan hasil pemeriksaan","end")]], chain(list("abcdef")), "Halaman Health hanya menampilkan status basis data, penyimpanan privat, scheduler, dan queue."),
]
...
def render_cover(c: canvas.Canvas) -> None:
    ...
    c.setFillColor(MINT)
    c.setFont(FONT,12)
    c.drawString(1.72*cm,H-8.0*cm,f"{len(DIAGRAMS)} diagram • pengembangan • sistem • penggunaan • administrasi")
...
def diagram_page(number: int) -> int:
    return number + 5
...
def render_toc(c: canvas.Canvas, items: list[tuple[int, Diagram]], page: int, title: str) -> None:
    ...
    for number,d in items:
        ...
        c.setFillColor(MUTED); c.drawRightString(W-1.5*cm,y,str(diagram_page(number)))
        y-=.46*cm
...
def render_legend(c: canvas.Canvas, page: int) -> None:
    ...
    notes="Panah menunjukkan urutan proses. Label Ya/Tidak atau status pada panah menunjukkan hasil keputusan. Diagram menggambarkan proses bisnis tingkat tinggi; rincian validasi mengikuti aturan bisnis proposal."
...
def build() -> None:
    ...
    toc_groups = (DIAGRAMS[:12], DIAGRAMS[12:24], DIAGRAMS[24:])
    start_number = 1
    for toc_page, diagrams in enumerate(toc_groups, 2):
        items = list(enumerate(diagrams, start_number))
        render_toc(c, items, toc_page, f"Daftar Diagram • Bagian {toc_page - 1}")
        c.showPage()
        start_number += len(diagrams)
