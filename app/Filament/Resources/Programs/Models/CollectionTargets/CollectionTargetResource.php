...
final class CollectionTargetResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('Definisi target')->schema([
                ...
                Repeater::make('scopes')->label('Scope target')->schema([
    ...
    public static function table(Table $table): Table
    {
        return $table->recordTitleAttribute('name')->columns([
            ...
            Action::make('activate')->label('Aktifkan')->icon(Heroicon::OutlinedPlay)->color('success')->visible(fn (CollectionTarget $record): bool => $record->status === TargetStatus::Draft)->authorize('activate')->requiresConfirmation()->action(fn (CollectionTarget $record): CollectionTarget => self::service()->activate(self::actor(), $record)),
            Action::make('close')->label('Tutup')->icon(Heroicon::OutlinedStop)->color('warning')->visible(fn (CollectionTarget $record): bool => $record->status === TargetStatus::Active)->authorize('close')->requiresConfirmation()->action(fn (CollectionTarget $record): CollectionTarget => self::service()->close(self::actor(), $record)),
            Action::make('cancel')->label('Batalkan')->icon(Heroicon::OutlinedXCircle)->color('danger')->visible(fn (CollectionTarget $record): bool => in_array($record->status, [TargetStatus::Draft, TargetStatus::Active], true))->authorize('cancel')->requiresConfirmation()->schema([Textarea::make('reason')->label('Alasan')->required()->minLength(10)->maxLength(1000)->rows(4)])->action(fn (CollectionTarget $record, array $data): CollectionTarget => self::service()->cancel(self::actor(), $record, (string) $data['reason'])),
        ]);
    }
