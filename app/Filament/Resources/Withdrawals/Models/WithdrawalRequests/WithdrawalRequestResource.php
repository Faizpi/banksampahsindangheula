        ...
        return $table
            ...
            ->recordActions([
                ...
                Action::make('assignPayer')
                    ...
                    ->authorize('approve')
                    ->schema([Select::make('payer_id')->label('Bendahara pembayaran')->helperText('Hanya bendahara aktif pada area nasabah yang dapat dipilih.')->options(fn (WithdrawalRequest $record): array => User::query()->where('users.status', 'aktif')->whereHas('roles', fn (Builder $roles): Builder => $roles->where('roles.name', 'bendahara'))->whereHas('staffProfile.serviceAreas', fn (Builder $assignment): Builder => $assignment->where(fn (Builder $dates): Builder => $dates->whereNull('staff_service_areas.active_from')->orWhereDate('staff_service_areas.active_from', '<=', today()))->where(fn (Builder $dates): Builder => $dates->whereNull('staff_service_areas.active_to')->orWhereDate('staff_service_areas.active_to', '>=', today()))->whereHas('serviceArea', fn (Builder $area): Builder => $area->where('service_areas.is_active', true)->whereHas('rts.customerProfiles', fn (Builder $customers): Builder => $customers->where('customer_profiles.user_id', $record->customer_id))))->orderBy('users.name')->pluck('users.name', 'users.id')->all())->required()])
