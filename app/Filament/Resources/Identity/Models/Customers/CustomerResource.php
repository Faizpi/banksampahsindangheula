    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordActions([
                ...
                Action::make('issueIdentity')->label('Terbitkan kartu/QR')->icon(Heroicon::OutlinedQrCode)->color('success')->authorize(fn (User $record): bool => auth()->user() instanceof User && auth()->user()->can('manageView', $record) && app(PermissionChecker::class)->allows(auth()->user(), 'customer.card.issue'))->visible(fn (User $record): bool => $record->customerProfile?->customer_number === null)->requiresConfirmation()->modalHeading('Terbitkan kartu dan QR')->modalDescription('Token QR dibuat sekali dan tidak ditampilkan pada panel. Kartu menyimpan QR secara aman melalui jalur presentasi yang diizinkan.')->action(function (User $record): void {
                    app(ManageCustomerIdentity::class)->issue(self::actor(), $record);
                })->successNotificationTitle('Kartu dan QR nasabah diterbitkan.'),
                Action::make('rotateQr')->label('Rotasi QR')->icon(Heroicon::OutlinedArrowPath)->color('warning')->authorize(fn (User $record): bool => auth()->user() instanceof User && auth()->user()->can('manageView', $record) && app(PermissionChecker::class)->allows(auth()->user(), 'customer.qr.rotate'))->visible(fn (User $record): bool => $record->customerProfile?->customer_number !== null)->requiresConfirmation()->schema([Textarea::make('reason')->label('Alasan rotasi')->required()->minLength(10)->maxLength(1000)->rows(3)])->action(function (User $record, array $data): void {
